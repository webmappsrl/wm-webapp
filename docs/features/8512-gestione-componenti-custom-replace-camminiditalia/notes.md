...
- **Scope esteso oltre l'overview/plan originali, su richiesta esplicita del developer durante la review-gate**, prima dei commit:
  1. Aggiunti `scripts/deploy-to-web-default.js` (mirror del vecchio comando inline `deploy`) e `scripts/deploy-to-web.js` (orchestratore che lancia in sequenza default + camminiditalia), replicando 1:1 il pattern a 3 script già usato in `webmapp-app` (`deploy-to-web-default.js` / `deploy-to-web-camminiditalia.js` / `deploy-to-web.js`). `package.json`: `deploy` ora è l'orchestratore, `deploy-default` il generico da solo, `deploy-camminiditalia` invariato.
