export const appPR = {
  "trackDetails": {
    "technicalData": {
      "ascent": "Elevação positiva",
      "descent": "Elevação negativa",
      "ele_from": "Altitude de partida",
      "ele_to": "Altitude de chegada",
      "ele_min": "Altitude mínima",
      "ele_max": "Altitude máxima",
      "duration": "Duração",
      "duration_forward": "Duração de ida",
      "duration_backward": "Duração de volta",
      "distance": "Distância",
      "difficulty": "Dificuldade"
    },
    "downloadUrls": {
      "gpx": "Baixar a trilha GPX",
      "kml": "Baixar a trilha KML",
      "geojson": "Baixar a trilha GEOJSON"
    },
    "description": {
      "readAll": "Ler tudo",
      "hide": "Ocultar"
    },
    "elevationChart": {
      "slope": {
        "label": "Inclinação"
      },
      "surfaces": {
        "asphalt": {
          "label": "Asfalto"
        },
        "concrete": {
          "label": "Concreto"
        },
        "dirt": {
          "label": "Terra"
        },
        "grass": {
          "label": "Grama"
        },
        "gravel": {
          "label": "Cascalho"
        },
        "paved": {
          "label": "Pavimentado"
        },
        "sand": {
          "label": "Areia"
        }
      }
    }
  },
  "home": {
    "duration": "Duração",
    "length": "Distância",
    "difference": "Elevação"
  },
  "skitouring": "Esqui alpino",
  "horse": "A cavalo",
  "walking": "Caminhada",
  "running": "Corrida",
  "cycling": "Ciclismo",
  "Punto di partenza": "Ponto de partida",
  "Punto di arrivo": "Ponto de chegada",
  "Dettagli tecnici": "Detalhes técnicos",
  "Attività": "Atividades",
  "Punti di interesse": "Pontos de interesse",
  "Collegamenti esterni": "Links externos",
  "Filtri": "Filtros",
  "where": "Locais",
  "Spiacenti non ci sono risultati con questi criteri di ricerca": "Desculpe, não há resultados com estes critérios de pesquisa",
  "Inserisci il nuovo nome": "Insira o novo nome",
  "Si è verificato un errore durante il salvataggio del percorso. Riprova": "Ocorreu um erro ao salvar o track. Tente novamente",
  "Il percorso è stato salvato correttamente": "O track foi salvo corretamente",
  "Per favore, inserisci un nome per el track": "Por favor, insira um nome para o track",
  "Errore": "Erro",
  "Clicca sulla mappa per avviare la creazione di un POI": "Clique no mapa para iniciar a criação de um POI",
  "privacy": {
    "agree": {
      "title": "Tratamento de dados",
      "message": "Consento que meus dados sejam publicados",
      "read_privacy": "Ler política de privacidade sobre o tratamento de dados",
      "accept": "Aceitar",
      "reject": "Rejeitar",
      "confirm": {
        "title": "Confirmar modificação",
        "accept_message": "Tem certeza de que deseja aceitar o tratamento de dados?",
        "reject_message": "Tem certeza de que deseja rejeitar o tratamento de dados?",
        "yes": "Sim, confirmar",
        "no": "Cancelar"
      },
      "final_confirm": {
        "title": "Modificação concluída",
        "accept_message": "O consentimento para o tratamento de dados foi aceito com sucesso.",
        "reject_message": "O consentimento para o tratamento de dados foi rejeitado com sucesso.",
        "ok": "OK"
      }
    }
  }
};
