...
describe('CommunicationService', () => {
  ...
  const DEF_DATA: string = 'DEF_DATA';
  const DEF_OPTIONS: string = 'DEF_OPTIONS';
  const DEF_RESULT: any = 'DEF_RESULT';
  const DEF_ERROR: any = 'DEF_ERROR';
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [CommunicationService, {provide: HttpClient, useValue: httpClientSpy}],
    });
  ...
  describe('post', () => {
    it('should resolve when there is no error', done => {
      httpClientSpy.post.and.callFake((url: string, data: any, options?: any) => {
        return new Observable(observer => {
          observer.next(DEF_RESULT);
        });
      });
      communicationService.post(DEF_URL, DEF_DATA, DEF_OPTIONS).subscribe(
        result => {
          ...
          expect(httpClientSpy.post).toHaveBeenCalledWith(DEF_URL, DEF_DATA, DEF_OPTIONS);
          expect(result).toEqual(DEF_RESULT);
          done();
        },
        err => {
